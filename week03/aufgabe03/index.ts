...
class Dog {
    ...
    constructor(name: string, points: number, breed: DogType) {
        this.name = name;
        this.points = points;
        this.breed = breed;
    }

    static getValidBreed(): DogType {
        while (true) {
            const typeInput = readlineSync.question("Welche Art ist Ihr Hund: ");
            if (Object.values(DogType).includes(typeInput as DogType)) {
                return typeInput as DogType;
            } else {
...
function getDogData() {
    ...
    const readlineSync = require("readline-sync");
    const dogsNum: number = parseInt(readlineSync.question("Wie viele Hunde möchten Sie erfassen: "));
    ...
    for (let i = 0; i < dogsNum; i++){
        ...
        const breed = Dog.getValidBreed();
        const dog = new Dog(dogName, dogPoints, breed);
...
result.dogData.forEach(dog => {
    console.log(`${dog.name} (${dog.breed}) erzielte ${dog.points} Punkte`);
});
